...
void APU::period_clock() {
    float sample = 0;
    float volume = mem.ppu_read(NR50) & 0b111; // currently only taking right ear volume into account
    volume = (volume + 1) / 8;
    if ((mem.ppu_read(NR10) & 0b1110000) == 0) { // pulse pace disabled
        ch1.pulse_pace = 0;
    } else if (ch1.pulse_pace == 0) { // internal pulse pace is 0, and new pace is written 
        ch1.pulse_pace = mem.ppu_read(NR10) >> 4;
    }
    ...
    if (ch1.enabled && (mem.ppu_read(NR12) >> 3) == 0) {
        disableChannel(1);
        ch2.dac = false;
    } else {
        ch1.dac = true;
    ...
    if (ch2.enabled && (mem.ppu_read(NR22) >> 3) == 0) {
        disableChannel(2);
        ch2.dac = false;
    } else {
        ch2.dac = true;
    }
    if (ch4.enabled && (mem.ppu_read(NR42) >> 3) == 0) {
        disableChannel(4);
        ch4.dac = false;
    } else {
        ch4.dac = true;
    }
    if (ch3.enabled && (mem.ppu_read(NR30) >> 7) == 0) {
        disableChannel(3);
        ch3.dac = false;
    } else {
        ch3.dac = true;
    ...
    u8 ch2_wave_duty = mem.ppu_read(NR21) >> 6;
    ...
    if (sample_counter >= 1048576) {
        ...
        if ((mem.ppu_read(NR52) & 0b10000000) == 0) {
            ch1.duty_step = 0;
            ch2.duty_step = 0;
            ch3.duty_step = 0;
            ch4.duty_step = 0;
    ...
    if (ch1.period_timer == 0x7FF) {
        ch1.period_timer = mem.ppu_read(NR13) + ((mem.ppu_read(NR14) & 0b111) << 8);
    ...
    for (auto i = 0; i < 2; ++i) {
        if (ch3.period_timer == 0x7FF) {
            ch3.period_timer = mem.ppu_read(NR33) + ((mem.ppu_read(NR34) & 0b111) << 8);
    ...
    if (ch4_tick == 3) {
        if (ch4.period_timer == ch4.clock_pace) {
            auto divider = (float)((int)(mem.ppu_read(NR43) & 0b111));
            divider = divider != 0 ? divider : 0.5;
            u16 shift = 2 << (mem.ppu_read(NR43) >> 4);
...
void APU::triggerCH2() {
    ch2.enabled = true;
    mem.ppu_write(NR52, (u8)(mem.ppu_read(NR52) | 0b10));
    ch2.internal_volume = mem.ppu_read(NR22) >> 4;
    ch2.length_timer = mem.ppu_read(NR21) & 0b111111;
    ch2.env_dir = 1 & (mem.ppu_read(NR22) >> 3);
...
void APU::triggerCH1() {
    ch1.enabled = true;
    mem.ppu_write(NR52, (u8)(mem.ppu_read(NR52) | 0b1));
    ch1.internal_volume = mem.ppu_read(NR12) >> 4;
    ch1.length_timer = mem.ppu_read(NR11) & 0b111111;
    ch1.pulse_pace = mem.ppu_read(NR10) >> 4;
    ch1.env_dir = 1 & (mem.ppu_read(NR12) >> 3);
...
void APU::triggerCH3() {
    ch3.enabled = true;
    mem.ppu_write(NR52, (u8)(mem.ppu_read(NR52) | 0b100));
    ch3.internal_volume = mem.ppu_read(NR32) >> 5;
    ch3.length_timer = mem.ppu_read(NR31);
    ch3.duty_step = 1;
}

void APU::triggerCH4() {
    ch4.enabled = true;
    mem.ppu_write(NR52, (u8)(mem.ppu_read(NR52) | 0b1000));
    ch4.internal_volume = mem.ppu_read(NR42) >> 4;
    ch4.length_timer = mem.ppu_read(NR41) & 0b111111;
    ch4.env_dir = 1 & (mem.ppu_read(NR42) >> 3);
...
void APU::envelopeAdjust() {
    ...
    if (ch2.env_sweep_tick != 0 && ch2.env_sweep_tick == (mem.ppu_read(NR22) & 0b111)) {
        ch2.env_sweep_tick = 0;
        if (ch2.env_dir == 1) {
    ...
    if (ch1.env_sweep_tick != 0 && ch1.env_sweep_tick == (mem.ppu_read(NR12) & 0b111)) {
        ch1.env_sweep_tick = 0;
        if (ch1.env_dir == 1) {
    ...
    if (ch4.env_sweep_tick != 0 && ch4.env_sweep_tick == (mem.ppu_read(NR42) & 0b111)) {
        ch4.env_sweep_tick = 0;
        if (ch4.env_dir == 1) {
...
void APU::lengthAdjust() {
    if ((mem.ppu_read(NR14) & 0b1000000) > 0) {
        ch1.length_timer -= 1;
        if (ch1.length_timer == 0) {
            disableChannel(1);
        }
    }
    if ((mem.ppu_read(NR24) & 0b1000000) > 0) {
        ch2.length_timer -= 1;
        if (ch2.length_timer == 0) {
            disableChannel(2);
        }
    }
    if ((mem.ppu_read(NR34) & 0b1000000) > 0) {
        ch3.length_timer -= 1;
        if (ch3.length_timer == 0) {
            disableChannel(3);
        }
    }
    if ((mem.ppu_read(NR44) & 0b1000000) > 0) {
        ch4.length_timer -= 1;
...
void APU::periodSweep() {
    ...
    if (ch1.pulse_timer != 0 && ch1.pulse_timer == ch1.pulse_pace) {
        u16 current_period =  mem.ppu_read(NR13) + ((mem.ppu_read(NR14) & 0b111) << 8);
        u16 period_diff = current_period / (1 << (mem.ppu_read(NR14) & 0b111));
        if ((mem.ppu_read(NR14) & 0b1000) > 0) { // subtraction
        ...
        mem.ppu_write(NR13, (u8)(current_period & 0xFF));
        mem.ppu_write(NR14, (u8)((mem.ppu_read(NR14) & 0b11000000) + (current_period >> 8)));
        ch1.pulse_pace = mem.ppu_read(NR10) >> 4;
        ch1.pulse_timer = 0;
    }
}
...
void APU::disableChannel(u8 channel) {
    switch (channel) {
        case 1:
            ch1.enabled = false;
            mem.ppu_write(NR52, (u8)(mem.ppu_read(NR52) & 0b11111110));
            break;
        case 2:
            ch2.enabled = false;
            mem.ppu_write(NR52, (u8)(mem.ppu_read(NR52) & 0b11111101));
            break;
        case 3:
            ch3.enabled = false;
            mem.ppu_write(NR52, (u8)(mem.ppu_read(NR52) & 0b11111011));
            break;
        case 4:
            ch4.enabled = false;
            mem.ppu_write(NR52, (u8)(mem.ppu_read(NR52) & 0b11110111));
            break;
    }
}

u8 APU::getNibble() {
    u8 byte = mem.ppu_read(WAVE_RAM_START + ch3.duty_step / 2);
...
void APU::lfsrClock() {
    ...
    ch4.lfsr &= ~(0b1 << 15);
    ch4.lfsr |= (new_bit << 15);
    if ((mem.ppu_read(NR43) & 0b1000) > 0) { // short lfsr
        ch4.lfsr &= ~(0b1 << 7);
        ch4.lfsr |= (new_bit << 7);
    }
